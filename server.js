...
// Import routes and services (but don't start them yet)
...
// Connect to MongoDB and initialize services AFTER connection is established
...
.then(async () => {
  ...
  // Initialize Redis after DB is connected
  try {
  ...
  // Start email monitoring ONLY after DB is ready
  console.log('🔄 Starting email monitoring service...');
  emailService.startMonitoring(io);

  // Start queue processor ONLY after DB is ready
...
.catch(err => {
  ...
  console.error('\n   CRITICAL: Server will NOT start email monitoring until MongoDB is connected.\n');
